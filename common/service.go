...
func ApplyService(clientset kubernetes.Clientset, new_service core_v1.Service) {
	...
	// 查询k8s是否有该service
	service, err := clientset.CoreV1().Services("default").Get(context.TODO(), new_service.Name, meta_v1.GetOptions{})
	if err != nil {
		...
		// 不存在则创建
		_, err = clientset.CoreV1().Services("default").Create(context.TODO(), &new_service, meta_v1.CreateOptions{})
		if err != nil {
			fmt.Println(err)
			return
		}
		...
		_, err = clientset.CoreV1().Services("default").Update(context.TODO(), &new_service, meta_v1.UpdateOptions{})
		if err != nil {
			fmt.Println(err)
			return
		}
