...
func ApplyDeployment(clientset kubernetes.Clientset, deployment apps_v1.Deployment) bool {
	...
	// 查询k8s是否有该deployment
	_, err := clientset.AppsV1().Deployments(namespace).Get(context.TODO(), deployment.Name, meta_v1.GetOptions{})
	if err != nil {
...
func DeleteDeployment(clientset kubernetes.Clientset, deployment apps_v1.Deployment) {
	...
	err := clientset.AppsV1().Deployments(namespace).Delete(context.TODO(), deployment.Name, meta_v1.DeleteOptions{})
	if err != nil {
		fmt.Println(err)
		return
	}
...
func GetDeploymentStatus(clientset kubernetes.Clientset, deployment apps_v1.Deployment) (success bool, reasons []string, err error) {

	// 获取deployment状态
	k8sDeployment, err := clientset.AppsV1().Deployments("default").Get(context.TODO(), deployment.Name, meta_v1.GetOptions{})
	if err != nil {
	...
	podList, err := clientset.CoreV1().Pods("default").List(context.TODO(), meta_v1.ListOptions{LabelSelector: labelSelector})
	if err != nil {
...
func PrintDeploymentStatus(clientset kubernetes.Clientset, deployment apps_v1.Deployment) {
